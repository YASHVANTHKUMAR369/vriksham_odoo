...
class HrEmployee(models.Model):
    ...
    def _compute_announcement_count(self):
        ...
        for employee in self:
            announcement_ids_general = employee.env[
                'hr.announcement'].sudo().search_count(
                [('is_announcement', '=', True),
                 ('state', '=', 'approved'),
                 ('date_start', '<=', fields.Date.today())])
            announcement_ids_emp = (employee.env['hr.announcement'].
            sudo().search_count(
                [('employee_ids', 'in', employee.id),
                 ('state', '=', 'approved'),
                 ('date_start', '<=', fields.Date.today())]))
            announcement_ids_dep = (employee.env['hr.announcement'].
            sudo().search_count(
                [('department_ids', 'in', employee.department_id.id),
                 ('state', '=', 'approved'),
                 ('date_start', '<=', fields.Date.today())]))
            announcement_ids_job = (employee.env['hr.announcement'].
            sudo().search_count(
